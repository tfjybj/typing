...
export default {
  ...
  methods: {
    ...
    Echarts_list() {
      this.$axios
        .post("/personResult/selectClassLevelCount", this.className)
        .then((res) => {
          var i = 0;
    ...
    initGrowthData() {
      ...
      this.className1.push(a);

      vm.$axios
        .post("/teamDetailResult/selectTeamGrowthChart", this.className1)
        .then((res) => {
          var i = 0;
          if (res.data.code == "0000") {
            for (i = 0; i < res.data.data.length; i++) {
