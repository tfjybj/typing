<!--
 * @Descripttion: 教师端键位查看邀请助力次数
 * @version: 
 * @Author: 张艳伟
 * @Date: 2021年10月5日22:51:33
 * @LastEditors: 
 * @LastEditTime: 
 -->
<template>
      ...
      <el-tabs>
        <div style="width: 100%">
          ...
          <div class="center">
            ...
            <el-button
              type="primary"
              @click="exportBattleTeamNumExcel"
           ...
           <el-button
              type="primary"
              @click="(addPersonDialogVisible = false), queryAllImproved()">查询总数据</el-button>    
              </div>
          <el-table
            :data="timeNumTableData"
            id="timeNumTableData"
            style="width: 100%"
            height="500"
          >
        ...
        <!-- 弹出框显示的内容：查看总数据 -->
          <el-dialog
            ...
            width="700px"
            >
           <el-button type="primary" @click="exportKeyExcel" style="margin-bottom: 10px" >导出</el-button>
            <el-table :data="selectTableData"  id="selectTableData" style="width: 100%" height="500">
              <el-table-column
                prop="userCode"
                label="学号"
                width="100"
...
export default {
  ...
  methods: {
    ...
    exportBattleTeamNumExcel() {
      if(this.timeNumTableData == ""){
        this.$message({
          message: "当前内容为空，不能导出",
          type: "warning",
        });
      }else{
        this.exportBattleTeamNumKey(); //导出时间排行榜数据
      }
    },
    exportBattleTeamNumKey() {
      /* generate workbook object from table */
      var xlsxParam = { raw: true }; // 导出的内容只做解析，不进行格式转换
      var wb = XLSX.utils.table_to_book(
        document.querySelector("#timeNumTableData"),
        xlsxParam
      );
    ...
    queryAllImproved() {
      ...
      if (this.selectMenu == "全部") {
        ...
        //查询后端接口
        vm.$axios
          .get("/personResult/selectAllImproved/" + this.startTime + "/" + this.endTime)
        ...
        vm.$axios
          .get(
            "/personResult/selectAllImprovedByClass/" +
              this.startTime +
              "/" +
              this.endTime +
              "/" +
    ...
    selectTime(startTime, endTime, selectContent) {
      if (this.selectMenu == "全部") {
        ...
        vm.$axios
          .get("/personResult/selectImproved/" + startTime + "/" + endTime)
          .then(function (response) {
            // console.log(response);
            if (response.data.code === "0000") {
              vm.total= response.data.data.length;
              vm.timeNumTableData = response.data.data;
            } else if (response.data.code === "1111") {
              // vm.$message({
              //   message: "暂无数据",
              //   type: "warning",
        ...
        vm.$axios
          .get(
            "/personResult/selectImprovedByClass/" +
              startTime +
              "/" +
              endTime +
              "/" +
